...
export default function Upload() {
  ...
  const {PostData, message, loading, setLoading} = usePost(`http://127.0.0.1:5000/api/posts`);
  ...
  try {
    ...
    const cloudRes = await fetch("https://api.cloudinary.com/v1_1/dtzodxlmb/image/upload", {
      method: "POST",
      body: data,
    });
  ...
  return (
    ...
    <div className="max-w-xl mx-auto px-10 py-8 bg-gray-100 rounded-xl mt-10">
      ...
      <div className="mb-5 flex items-center justify-center w-full">
        ...
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            {selectedFile && (
              <span className="mt-2 text-xl text-orange font-semibold">{selectedFile.name}</span>
            )}
          </div>
      ...
      >
        ...
        <Button content={loading?"Uploading...":"Upload"} type="submit" className="w-full col-span-2" />
      </form>
    </div>
    </div>
  );
